...
private CategoryPrint categoryPrint=new CategoryPrint();
    ...
    public void selectCategoryList(Map<String, String> parameter) {
        ...
        if (categoryList != null){
            ...
            categoryPrint.printErrorMessage("null");
        }
        // 2. 제품분류 목록을 조회하는 메소드
    ...
    public void registNewCategory(CategoryDTO category) {
        ...
        categoryService1.registNewCategory(category);

        boolean result= categoryService1.registNewCategory(category);


        if (result){
            categoryPrint.printSuccessMessage("success");
        }else {
            categoryPrint.printErrorMessage("fall");
        }
